...
export class AtsuMoeScraper extends BaseScraper {
  ...
  async search(query: string): Promise<SearchResult[]> {
    const searchUrl = `${
      this.BASE_URL
    }/collections/manga/documents/search?q=${encodeURIComponent(
      query
    )}&limit=12&query_by=title%2CenglishTitle%2CotherNames&query_by_weights=3%2C2%2C1&include_fields=id%2Ctitle%2CenglishTitle%2Cposter&num_typos=4%2C3%2C2`;
  ...
  private extractMangaId(url: string): string | null {
    const mangaMatch = url.match(
      new RegExp(`/manga/(${this.ATSU_ID_PATTERN})`)
    );
    if (mangaMatch) {
      return mangaMatch[1];
    }
  ...
  override async getPages(
    chapterUrl: string
  ): Promise<{ image: number; url: string }[]> {
    const [_, mangaId, chapterId] =
      /https:\/\/atsu\.moe\/read\/(.+)\/(.+)/g.exec(chapterUrl)!;

    const pages = await this.fetchWithRetry(
      `${this.BASE_URL}/api/read/chapter?mangaId=${mangaId}&chapterId=${chapterId}`
    );

    const data = JSON.parse(pages);

    return data.readChapter.pages.map(
      (c: { id: string; image: string; number: number }) => {
        return { image: `${this.BASE_URL}${c.image}`, number: c.number };
      }
    );
  }
}
