...
export class KaliScanScraper extends BaseScraper {
  ...
  async getChapterList(mangaUrl: string): Promise<ScrapedChapter[]> {
    ...
    try {
      ...
      const apiUrl = `${
        this.BASE_URL
      }/service/backend/chaplist/?manga_id=${mangaId}&manga_name=${encodeURIComponent(
        mangaSlug
      )}`;
  ...
  protected override extractChapterNumber(chapterUrl: string): number {
    const patterns = [/chapter[/-](\d+(?:\.\d+)?)/i, /ch[/-](\d+(?:\.\d+)?)/i];
  ...
  override async getPages(
    ...
    const scriptContent = $("script")
      .map((_, el) => $(el).html() ?? "")
      .get()
      .find((s) => s.includes("chapImages"));

    if (!scriptContent) return [];

    const match = scriptContent.match(/var chapImages\s*=\s*"([^"]+)"/);
    if (!match) return [];

    return match[1]
      .split(",")
      .map((url, i) => ({ image: i + 1, url: url.trim() }));
  }
}
