...
export class KenscansScraper extends BaseScraper {
  ...
  async search(query: string): Promise<SearchResult[]> {
    try {
      const searchUrl = `${
        this.API_URL
      }/api/query?page=1&perPage=24&searchTerm=${encodeURIComponent(
        query
      )}&seriesType=&seriesStatus=`;
      ...
      for (const series of limitedSeries) {
        try {
          ...
          $("a[href*='/chapter-']").each((_: number, element: any) => {
            ...
            const hasLockIcon =
              $link.find('svg path[d*="5.25 5.25"]').length > 0;
            if (hasLockIcon) {
              return;
            }
  ...
  override async getPages(
    ...
    const pages: { image: number; url: string }[] = [];

    $(".reading-content")
      .find('img[id^="image"]')
      .each((_, el) => {
        const src = $(el).attr("src");
        const index = parseInt($(el).attr("id")?.split("image-").pop()!);

        if (src) {
          pages.push({
            image: index,
            url: new URL(src as any).toString(),
          });
        }
      });
